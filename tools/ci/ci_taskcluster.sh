...
if [ $1 == "firefox" ]; then
    ./wpt run firefox --log-wptreport=../artifacts/wpt_report.json --log-mach=- --log-mach-level=info --this-chunk=$3 --total-chunks=$4 --test-type=$2 -y --install-browser --no-pause --no-restart-on-unexpected --reftest-internal --install-fonts --no-fail-on-unexpected --include encoding
elif [ $1 == "chrome" ]; then
